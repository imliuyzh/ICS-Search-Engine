...
def parse(toParse: dict) -> ([str], [str]):
    """Given a HTML string and parse it into lists of tokens."""
    soup = BeautifulSoup(toParse['content'], 'lxml')
    
    importantToken = []     # Bolded words, headings, title...
    importantCandidate = soup.findAll(["title", "strong", "b", "h1", "h2",
        "h3", "h4", "h5", "h6"])
    for x in importantCandidate:
        importantToken.extend(tokenize(x.extract().get_text()))
    
    removeNoise(soup)
    normalToken = tokenize(soup.get_text())     # Words other than those above
